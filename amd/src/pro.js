...
define(['jquery', 'core/log', 'core/config'], function($, log, cfg) {
    ...
    var tabbedquickformpro = {

        init: function() {

            $('#id_s_local_tabbedquickform_licensekey').bind('change', this.check_product_key);
            $('#id_s_local_tabbedquickform_licensekey').trigger('change');
            log.debug('AMD Pro js initialized for local tabbedquickform');
        },
        ...
        check_product_key: function() {
            ...
            var calculated = tabbedquickformpro.checksum(payload);
            ...
            if (crc === calculated) {
                var url = cfg.wwwroot + '/local/tabbedquickform/pro/ajax/services.php?';
                url += 'what=license';
                url += '&service=check';
                url += '&customerkey=' + that.val();
                url += '&provider=' + $('#id_s_local_tabbedquickform_licenseprovider').val();

                $('#id_s_local_tabbedquickform_licensekey + img').remove();
                $('#id_s_local_tabbedquickform_licensekey').after(waiticon);

                $.get(url, function(data) {
                    if (data.match(/SET OK/)) {
                        $('#id_s_local_tabbedquickform_licensekey + img').remove();
                        $('#id_s_local_tabbedquickform_licensekey').after(validicon);
                    } else {
                        $('#id_s_local_tabbedquickform_licensekey + img').remove();
                        $('#id_s_local_tabbedquickform_licensekey').after(invalidicon);
                    }
                }, 'html');
            } else {
                $('#id_s_local_tabbedquickform_licensekey + img').remove();
                $('#id_s_local_tabbedquickform_licensekey').after(cautionicon);
            }
        },
